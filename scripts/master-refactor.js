        ...
        try {
            ...
            if (result) {
                ...
                const msg = result.type === 'standardize imports' 
                    ? `refactor(code): standardize imports in ${fileName}`
                    : result.type === 'add JSDoc'
                        ? `docs: add JSDoc to functions in ${fileName}`
                    : result.type === 'add displayName'
                        ? `refactor(react): add displayName to ${fileName}`
                    : `refactor(js): modernize variables in ${fileName}`;
